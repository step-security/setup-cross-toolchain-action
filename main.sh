      ...
      case "${qemu_arch}" in
        ...
        i386)
          ...
          _sudo tee -- /proc/sys/fs/binfmt_misc/register >/dev/null \
            <<<":${target}-i386:M::${magic}:${mask}:${runner_path}:F"
          # i486
          magic='\x7fELF\x01\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x06\x00'
          mask='\xff\xff\xff\xff\xff\xfe\xfe\x00\xff\xff\xff\xff\xff\xff\xff\xff\xfe\xff\xff\xff'
          _sudo tee -- /proc/sys/fs/binfmt_misc/register >/dev/null \
            <<<":${target}-i486:M::${magic}:${mask}:${runner_path}:F"
          printf '::endgroup::\n'
          return
          ;;
        hexagon)
        ...
        x86_64)
          ...
          mask='\xff\xff\xff\xff\xff\xfe\xfe\x00\xff\xff\xff\xff\xff\xff\xff\xff\xfe\xff\xff\xff'
          ;;
        *) bail "internal error: unrecognized qemu arch '${qemu_arch}'" ;;
      esac
      ;;
    wasmtime)
      magic='\x00asm'
          ...
          *)
            ...
            case "${target}" in
              ...
              i686-* | x86_64*)
                ...
                if [[ "${codename}" == 'resolute' ]] && [[ "${wine_version}" == '10.0.0.0' ]]; then
                  apt_packages+=(wine wine64)
                elif [[ "${codename}" =~ 'noble' ]] && [[ "${wine_version}" == '9.0.0.0' ]]; then
